...
def critere_benef(file, ante=pd.to_datetime(str(start_date_entry.get()), dayfirst=True), post=pd.to_datetime(str(end_date_entry.get())), dayfirst=True):
    ...
    df_erreur_age = benef_sans_doublon[(benef_sans_doublon["statutAce"] == "A") & ((benef_sans_doublon['age'] <= 25) | (benef_sans_doublon['age'] < 0)) | ((benef_sans_doublon["statutAce"] == 'E') & (benef_sans_doublon['age'] < 0))]
    ...
    if len(benef_sans_doublon) != 0:
        ...
        path_sans_doublon = os.path.join(
            nom_dossier, sous_dossier, "SANS_DOUBLONS_BENEF.xlsx")
        benef_sans_doublon.to_excel(path_sans_doublon,sheet_name="SANS_DOUBLONS_BENEF")

    if len(benef_doublon) != 0:
        path_doublon = os.path.join(
            nom_dossier, sous_dossier, "DOUBLONS_BENEF.xlsx")
        benef_doublon.to_excel(path_doublon,sheet_name="DOUBLONS_BENEF")

    if len(police_benef) != 0:
        path_police_benef = os.path.join(
            nom_dossier, sous_dossier, "POLICE_BENEF.xlsx")
        police_benef.to_excel(path_police_benef,sheet_name="POLICE_BENEF")

    if len(ENFANT_SUP_25) != 0:
        path_ENFANT_SUP_25 = os.path.join(
            nom_dossier, sous_dossier, "ENFANTS_SUP_25_ANS.xlsx")
        ENFANT_SUP_25.to_excel(path_ENFANT_SUP_25,sheet_name="ENFANTS_SUP_25_ANS")

    if len(ADULTES_SUP_60) != 0:
        path_ADULTES_SUP_60 = os.path.join(
            nom_dossier, sous_dossier, "ADULTES_SUP_60_ANS.xlsx")
        ADULTES_SUP_60.to_excel(path_ADULTES_SUP_60,sheet_name="ADULTES_SUP_60_ANS")

    if len(benef_sans_date_naiss) != 0:
        path_benef_sans_date_naiss = os.path.join(
            nom_dossier, sous_dossier, "BENEF_SANS_DATE_NAISS.xlsx")
        benef_sans_date_naiss.to_excel(path_benef_sans_date_naiss,sheet_name="BENEF_SANS_DATE_NAISS")

   
    if len(df_erreur_age) != 0:
        path_df_erreur_age = os.path.join(nom_dossier, sous_dossier, "BENEF_AGE_ERRONE.xlsx")
        df_erreur_age[df_erreur_age["numPolice"].isin(liste_errs)].to_excel(path_df_erreur_age,sheet_name="BENEF_AGE_ERRONE")
...
def critere_emission(file, post=pd.to_datetime(str(end_date_entry.get()), dayfirst=True)):
    ...
    if len(doublons_emission) != 0:
        path_doublon = os.path.join(nom_dossier, sous_dossier, "DOUBLONS_COTISATION.xlsx")
        doublons_emission.to_excel(path_doublon, sheet_name="DOUBLONS_COTISATION")

    if len(sans_doublons_emission) != 0:
        path_sans_doublon = os.path.join(nom_dossier, sous_dossier, "SANS_DOUBLONS_COTISATION.xlsx")
        sans_doublons_emission.to_excel(path_sans_doublon, sheet_name="SANS_DOUBLONS_COTISATION")

    if len(police_emission) != 0:
        path_police_emission = os.path.join(nom_dossier, sous_dossier, "POLICE_COTISATION.xlsx")
        police_emission.to_excel(path_police_emission, sheet_name="POLICE_COTISATION")
    return
...
def critere_conso(file):
    ...
    doublons_conso = conso[conso.duplicated(subset=colonnes_doublons,keep=False)]
    ...
    if len(doublons_conso) != 0:
        path_doublon = os.path.join(
            nom_dossier, sous_dossier, "DOUBLONS_CONSO.xlsx")
    ...
    if len(sans_doublons_conso) != 0:
        path_sans_doublons_conso = os.path.join(
            nom_dossier, sous_dossier, "SANS_DOUBLONS_CONSO.xlsx")
        sans_doublons_conso.to_excel(path_sans_doublons_conso,sheet_name="SANS_DOUBLONS_CONSO")

    if len(police_conso) != 0:
        path_police_conso = os.path.join(
            nom_dossier, sous_dossier, "POLICE_CONSO.xlsx")
...
def resume(file_benef, file_emission, file_conso, seuil_ctx, ante=pd.to_datetime(str(start_date_entry.get())), post=pd.to_datetime(str(end_date_entry.get()))):
    ...
    if len(CONSO_COUTEUSES) != 0:
        path_CONSO_COUTEUSES = os.path.join(
            nom_dossier, sous_dossier, "CONSO_COUTEUSES.xlsx")
    ...
    Resume().ecart(selected_beneficiaire_path, selected_emission_path,selected_consommation_path,COUTIER_entry.get(),end_date_entry.get())
    return
