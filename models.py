...
class Resume:
    ...
    def ecart(self, file_benef, file_cotisation, file_conso, courtier,date_fin):
        ...
        sans_doublons_conso = consommation.drop_duplicates()
        ENFANT_SUP_25 = benef_sans_doublon[benef_sans_doublon["statutAce"].isin(['E']) & (benef_sans_doublon['age'] > 25)]
        ADULTES_SUP_60 = benef_sans_doublon[benef_sans_doublon["statutAce"].ne('E') & (benef_sans_doublon['age'] > 60)]

        cond1 = sans_doublons_conso['matricule'].isin(ENFANT_SUP_25["matricule"].unique())

        #4 CONSO ENFANTS DE PLUS DE 25 ANS
        ENFANT_SUP_25_CONSO = sans_doublons_conso[cond1]["numPolice"].apply(lambda x: str(x)).tolist()

        cond2 = sans_doublons_conso['matricule'].isin(ADULTES_SUP_60["matricule"].unique())

        #5 CONSO ADULTES DE PLUS DE 50 ANS
        ADULTES_SUP_60_CONSO = sans_doublons_conso[cond2]["numPolice"].apply(lambda x: str(x)).tolist()
        ...
        dble_conso = set(consommation[consommation.duplicated(subset=colonnes_doublons,keep=False)]["numPolice"].apply(lambda x: str(x)).tolist())
        ...
        df_erreur_age = sans_doublons_conso[cond]
        df_erreur_age.loc[:, "age"] = pd.to_datetime(benef_sans_doublon['dateNaissance'], dayfirst=True).apply(age)
        # print("=-"*25,"ERREUR AGE","=-"*25)
        # print(liste_errs)
        # print(df_erreur_age.head(5))

        #----------------FICHIER ECARTS 
        with pd.ExcelWriter(dossier_complet+'/'+'ECARTS.xlsx', engine='xlsxwriter') as writer:
            try:
                if len(presta_sans_cotisation) != 0:
                    consommation[consommation["numPolice"].isin(presta_sans_cotisation)].to_excel(writer, sheet_name="PRESTA_SANS_COTISATION", index=False)
            except:
                #print('PRESTA_SANS_COTISATION vide')
                pass
            try:
                if len(presta_hors_periode_couv) != 0:
                    consommation[consommation['numPolice'].isin(presta_hors_periode_couv)].to_excel(writer, sheet_name="PRESTA_HORS_PERIODE_CVT", index=False)
            except:
                #print('PRESTA_HORS_PERIODE_CVT vide')
                pass
            try:
                if len(ENFANT_SUP_25_CONSO) != 0:
                    consommation[consommation['numPolice'].isin(ENFANT_SUP_25_CONSO)].to_excel(writer, sheet_name="PRESTA_ENFANTS_SUP_25_ANS", index=False)
            except:
                #print('PRESTA_ENFANTS_SUP_25_ANS vide')
                pass
            try:
                if len(ADULTES_SUP_60_CONSO) != 0:
                    consommation[consommation['numPolice'].isin(ADULTES_SUP_60_CONSO)].to_excel(writer, sheet_name="PRESTA_ADULTES_SUP_60_ANS", index=False)
            except:
                #print('PRESTA_ADULTES_SUP_60_ANS vide')
                pass
            try:
                if len(benef_sans_date_naiss) != 0:
                    consommation[consommation['numPolice'].isin(benef_sans_date_naiss)].to_excel(writer, sheet_name="SANS_DATE_NAISS", index=False)
            except:
                #print('PRESTA_SANS_DATE_NAISS vide')
                pass
            try:
                if len(dble_conso) != 0:
                    consommation[consommation['numPolice'].isin(dble_conso)].to_excel(writer, sheet_name="DOUBLE_CONSOMMATION", index=False)
            except:
                #print('DOUBLE_CONSOMMATION vide')
                pass
            try:
                if len(liste_sans_date_emission) != 0:
                    consommation[consommation['numPolice'].isin(liste_sans_date_emission)].to_excel(writer, sheet_name="SANS_DATE_EMISSION", index=False)
            except:
                #print('SANS_DATE_EMISSION vide')
                pass
            try:
                if len(liste_hors_delai_contractuel) !=0:
                    consommation[consommation['numPolice'].isin(liste_hors_delai_contractuel)].to_excel(writer, sheet_name="HORS_DELAI_CNTCT", index=False)
            except:
                #print('PRESTA_HORS_DELAI_CONTRACTUEL vide')
                pass
            try:
                if len(liste_presta_hors_periode) != 0:
                    consommation[consommation["numPolice"].apply(lambda x: str(x).replace(" ","")).isin(liste_presta_hors_periode)].to_excel(writer, sheet_name="HORS_PERIODE_CTRL", index=False)
            except:
                #print('PRESTA_HORS_PERIODE vide')
                pass
            try:
                if len(liste_presta_dateSoins_apres_dateSortie) != 0:
                    consommation[consommation["numPolice"].apply(lambda x: str(x).replace(" ","")).isin(liste_presta_dateSoins_apres_dateSortie)].to_excel(writer, sheet_name="DSOINS_APR_DSORTIE", index=False)
            except:
                #print('DATE_SORTIE_APRES_DATE_SOINS vide')
                pass
            try:
                if len(liste_errs) != 0:
                    df = consommation[consommation["numPolice"].apply(lambda x: str(x).replace(" ","")).isin(liste_errs)]
                    df.loc[:, "age"] = pd.to_datetime(benef_sans_doublon['dateNaissance'], dayfirst=True).apply(age)
                    df.to_excel(writer, sheet_name="AGE_ERRONES ", index=False)
        ...
        with pd.ExcelWriter(dossier_complet + '/' + 'DETAILS_ECARTS.xlsx', engine='xlsxwriter') as writer_:
            try:
            ...
            try:
                if len(dble_conso) != 0:
                    df = consommation[consommation["numPolice"].isin(dble_conso)].groupby(['numPolice']).agg({'matricule':'count','mtReclame': 'sum', 'mtExclu': 'sum','baseRemboursement':'sum', 'ticketModerateur': 'sum', 'montantPaye': 'sum'})
                    totals = df.sum(numeric_only=True) 
                    df.loc['TOTAL'] = totals
                    df.reset_index(inplace=True)
            ...
            try:
                if len(liste_presta_hors_periode) != 0:
                    df = consommation[consommation["numPolice"].isin(liste_presta_hors_periode)].groupby(['numPolice']).agg({'matricule':'count','mtReclame': 'sum', 'mtExclu': 'sum','baseRemboursement':'sum', 'ticketModerateur': 'sum', 'montantPaye': 'sum'})
                    totals = df.sum(numeric_only=True) 
                    df.loc['TOTAL'] = totals
                    df.reset_index(inplace=True)
            ...
            try:
                if len(liste_errs) != 0:
                    ...
                    df.loc['TOTAL'] = totals
                    df.reset_index(inplace=True)
                    df.reset_index().to_excel(writer_, sheet_name="AGE_ERRONES", index=False)
